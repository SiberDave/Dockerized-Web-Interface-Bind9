                    ...
                    <table class="table w-full text-sm bg-white">
                        ...
                        <tbody v-else>
                            <tr class="table-row w-full">
                                <td class="table-cell" colspan="4">No Custom Domain</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            totalpage() {
                return Math.ceil(this.ListCustomDomain.length / this.totalitem)
            },
            totalpagefilter(){
        ...
        methods: {
            async fetchData() {
                try {
                    ...
                    const response = await axios.get(`http://${process.env.VUE_APP_HOST_API}:3000/list-custom-domain`);
                    this.ListCustomDomain = response.data
                } catch (error) {
            ...
            datafilter(){
                ...
                if (this.searchQuery.toString().trim().length == 0){
                    pagedata = this.ListCustomDomain
                }
                else{
